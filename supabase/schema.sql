...
create policy "Product product tags are viewable by everyone" on public.product_product_tags for select using (true);
create policy "Product ingredients are viewable by everyone" on public.product_ingredients for select using (true);
create policy "Product extra options are viewable by everyone" on public.product_extra_options for select using (true);
...
create policy "Users can view own dietary preferences" on public.user_dietary_preferences for select using (auth.uid() = user_id);
create policy "Users can manage own dietary preferences" on public.user_dietary_preferences for all using (auth.uid() = user_id);
...
create policy "Users can view own order item extras" on public.order_item_extras for select using (
  exists (select 1 from public.order_items join public.orders on orders.id = order_items.order_id where order_items.id = order_item_extras.order_item_id and orders.user_id = auth.uid())
);
create policy "Users can insert own order item extras" on public.order_item_extras for insert with check (
  exists (select 1 from public.order_items join public.orders on orders.id = order_items.order_id where order_items.id = order_item_extras.order_item_id and orders.user_id = auth.uid())
);

create policy "Users can view own order item customizations" on public.order_item_customizations for select using (
  exists (select 1 from public.order_items join public.orders on orders.id = order_items.order_id where order_items.id = order_item_customizations.order_item_id and orders.user_id = auth.uid())
);
create policy "Users can insert own order item customizations" on public.order_item_customizations for insert with check (
  exists (select 1 from public.order_items join public.orders on orders.id = order_items.order_id where order_items.id = order_item_customizations.order_item_id and orders.user_id = auth.uid())
);

create policy "Users can view own order delivery address" on public.order_delivery_addresses for select using (
  exists (select 1 from public.orders where orders.id = order_delivery_addresses.order_id and orders.user_id = auth.uid())
);
create policy "Users can insert own order delivery address" on public.order_delivery_addresses for insert with check (
  exists (select 1 from public.orders where orders.id = order_delivery_addresses.order_id and orders.user_id = auth.uid())
);
...
create policy "Users can insert own order payments" on public.order_payments for insert with check (
  exists (select 1 from public.orders where orders.id = order_payments.order_id and orders.user_id = auth.uid())
);
...
create policy "Authenticated avatar upload" on storage.objects for insert with check (
...
insert into public.stores (id, name, neighborhood, city, address, reference, phone, whatsapp, hours, is_open, latitude, longitude, image) values
...
insert into public.product_extra_options (id, name, price, category) values
...
insert into public.coupons (id, code, discount_type, discount_value, min_order_value, description, expires_at) values
...
insert into public.products (id, name, description, price, promotional_price, category_id, image, rating, sales_count, is_available, is_featured) values
